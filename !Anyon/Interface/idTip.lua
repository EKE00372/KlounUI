...
local M = F.RegisterModule("idTip", {"idTip_Opt1", "idTip_Opt2", "idTip_Opt3"})

------------
-- Option -- 
------------

-- 三個 idTip 子選項全部不選時不會註冊 tooltip，等同停用整個模組
...
function M:OnEnable()
	...
	local function isKindEnabled(kind)
		if primaryKinds[kind] then return IsOptionEnabled("idTip_Opt1") end
		if bonusTraitKinds[kind] then return IsOptionEnabled("idTip_Opt3") end
		return IsOptionEnabled("idTip_Opt2")
	end
